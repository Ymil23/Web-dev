...
app.get('/random', (req, res) => {
  ...
  const randomJ = Math.floor(Math.random() * jokes.length);
  res.json(jokes[randomJ]);
...
app.get('/jokes/:id', (req, res) => {
  ...
  const jokeFound = jokes.find((jo) => jo.id === id);
...
app.get('/filter', (req, res) => {
  ...
  const filteredJoke = jokes.filter((kaftan) =>
    kaftan.jokeType === type);

  if (!filteredJoke) {
    return res.json({
      error: `there isnt the joke type of ${type} `
    });
  }
...
app.put('/jokes/:id', (req, res) => {
  ...
  const replacementJoke = {
    id: id,
    jokeText: req.body.newText,
    jokeType: req.body.type

  };

  const badalkaRaadi = jokes.findIndex((kaftan) => kaftan.id === id);

  jokes[badalkaRaadi] = replacementJoke;
